...
// Pool used / free / total bar.  Returns used bytes.
...
void AudioDebugWindow::DrawElement() {
    ...
    // ── Frame Step ───────────────────────────────────────────────────────────
    {
        ...
        // Remaining N counter (shows how many frames are still queued)
    ...
    // ── PCM Capture ──────────────────────────────────────────────────────────
    if (ImGui::CollapsingHeader("PCM Capture", ImGuiTreeNodeFlags_DefaultOpen)) {
        // Output path input
    ...
    // ── Sequence Players ─────────────────────────────────────────────────────
    // Per-player live editable state (persists across frames in static arrays)
    ...
    if (ImGui::CollapsingHeader("Sequence Players", ImGuiTreeNodeFlags_DefaultOpen)) {
...
#undef ROW
            ...
            // Stop (fade out over 20 ticks) / Silence (instant vol=0) / Restore
    ...
    // ── Active Notes (all voices with full sample + loop info) ───────────────
    if (ImGui::CollapsingHeader("Active Notes", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        static float  sRefreshHz     = 4.f;   // updates per second (1–60)
        static double sLastRefreshTime = 0.0;

        // Capture a fresh snapshot from gNotes
        auto capture = [&]() {
            ...
            for (int ni = 0; ni < gNumNotes; ni++) {
                ...
                // Validate the full pointer chain against statically-allocated pools
                {
        ...
        // Decide whether to refresh this frame
    ...
    // ── Per-Channel Stats ────────────────────────────────────────────────────
    if (ImGui::CollapsingHeader("Channel Stats")) {
        // Show channels for each sequence player (even idle ones)
        for (int pi = 0; pi < SEQ_PLAYER_MAX; pi++) {
            const SequencePlayer& p = gSeqPlayers[pi];

            // Count valid (non-sentinel) channels first
            int validCount = 0;
            for (int ci = 0; ci < SEQ_NUM_CHANNELS; ci++) {
    ...
    // ── Note / Voice Polyphony ───────────────────────────────────────────────
    if (ImGui::CollapsingHeader("Polyphony", ImGuiTreeNodeFlags_DefaultOpen)) {
        // Count active notes by walking the global note array
        int activeCount   = 0;
        ...
        // Note-pool free-list counts — walk linked lists (no count field)
...
#undef RROW
    ...
    // ── Audio Heap ───────────────────────────────────────────────────────────
    if (ImGui::CollapsingHeader("Audio Heap")) {
