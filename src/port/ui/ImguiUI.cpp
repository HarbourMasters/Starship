...
void DrawSpeakerPositionEditor() {
    ...
    if (!initialized || (lastCanvasPos.x != canvasPos.x || lastCanvasPos.y != canvasPos.y)) {
        const char* cvarNames[4] = { "gPositionFrontLeft", "gPositionFrontRight", "gPositionRearLeft",
                                     "gPositionRearRight" };
    ...
    for (int i = 0; i < 4; i++) {
        ...
        drawList->AddText(ImVec2(speakerPositions[i].x - 6, speakerPositions[i].y - 6), IM_COL32(255, 255, 255, 255),
                          speakerLabels[i]);
        ...
        if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
            ...
            ImVec2 constrainedPos =
                ImVec2(center.x + (direction.x / length) * radius, center.y + (direction.y / length) * radius);
            ...
            float updatedAngle =
                atan2f(speakerPositions[i].y - center.y, speakerPositions[i].x - center.x) * (180.0f / M_PI);
            if (updatedAngle < 0)
                updatedAngle += 360.0f;
...
void DrawSettingsMenu() {
        ...
        if (UIWidgets::BeginMenu("Audio")) {
            ...
            if (CVarGetInteger("gAudioChannelsSetting", 0) == 1) {
                ...
                UIWidgets::CVarSliderInt("Subwoofer threshold (Hz)", "gSubwooferThreshold", 10u, 1000u, 80u,
                                         {
                                             .tooltip = "The threshold for the subwoofer to be activated. Any sound "
                                                        "under this frequency will be played on the subwoofer.",
                                             .format = "%d",
                                         });
                    ...
                    if (UIWidgets::Button("Install JP/EU Audio")) {
                        if (GameEngine::GenAssetFile(false)) {
                            GameEngine::ShowMessage(
                                "Success", "Audio assets installed. Changes will be applied on the next startup.",
    ...
    if (UIWidgets::BeginMenu("Graphics")) {
        ...
        { // FPS Slider
            ...
            UIWidgets::CVarSliderInt((currentFps == 30) ? "FPS: Original (30)" : "FPS: %d", "gInterpolationFPS", minFps,
                                     maxFps, 60, { .disabled = matchingRefreshRate });
#endif
            UIWidgets::Tooltip(
        ...
        if (Ship::Context::GetInstance()->GetWindow()->CanDisableVerticalSync()) {
            UIWidgets::PaddedEnhancementCheckbox("Enable Vsync", "gVsyncEnabled", true, false, false, "",
        ...
        UIWidgets::PaddedEnhancementCheckbox("Apply Point Filtering to UI Elements", "gHUDPointFiltering", true, false,
...
void DrawMenuBarIcon() {
    ...
    if (!gameIconLoaded) {
        // Ship::Context::GetInstance()->GetWindow()->GetGui()->LoadGuiTexture("Game_Icon", "textures/icons/gIcon.png",
...
static const char* radioCommBox[] = { "Original", "Expand" };
...
void DrawEnhancementsMenu() {
    if (UIWidgets::BeginMenu("Enhancements")) {
        ...
        if (UIWidgets::BeginMenu("Gameplay")) {
            UIWidgets::CVarCheckbox(
                ...
                { .tooltip = "Disable Level of Detail (LOD) to avoid models using lower poly versions at a distance",
                  .defaultValue = true });
            UIWidgets::CVarCheckbox(
                "Character heads inside Arwings at all times", "gTeamFaces",
                { .tooltip = "Character heads are displayed inside Arwings in all cutscenes", .defaultValue = true });
        ...
        if (UIWidgets::BeginMenu("Fixes")) {
            ...
            UIWidgets::CVarCheckbox(
                "Sector Z: Spawn all actors", "gSzActorFix",
                { .tooltip = "Fixes a bug found in Sector Z, where only 10 of 12 available actors are spawned, this "
                             "causes two 'Space Junk Boxes' to be missing from the level." });
        ...
        if (UIWidgets::BeginMenu("HUD")) {
            if (UIWidgets::CVarCombobox(
                    "Radio Communication Box", "gRadioCommBox.Selection", radioCommBox,
                    {
            ...
            if (UIWidgets::CVarCombobox(
                    "HUD Aspect Ratio", "gHUDAspectRatio.Selection", hudAspects,
                    {
        ...
        if (UIWidgets::BeginMenu("Accessibility")) {
            UIWidgets::CVarCheckbox(
                ...
                { .tooltip = "Gorgon flashes the screen repeatedly when firing its beam or when teleporting, which "
                             "causes eye pain for some players and may be harmful to those with photosensitivity.",
                  .defaultValue = false });
            UIWidgets::CVarCheckbox(
                "Add outline to Arwing and Wolfen in radar", "gFighterOutlines",
                { .tooltip = "Increases visibility of ships in the radar.", .defaultValue = false });
            ImGui::EndMenu();
        }
...
void DrawModsMenu() {
    if (UIWidgets::BeginMenu("Mods")) {
        ...
        for (const auto& entry : *mods) {
            if (!entry->GetManifest().Name.empty()) {
                hasAnyMod = true;
                break;
            }
        }

        if (UIWidgets::BeginMenu(ICON_FA_PUZZLE_PIECE " List")) {
            if (!hasAnyMod) {
                ...
                for (const auto& entry : *mods) {
                    ...
                    // Mod name header
                    std::string nameLabel = info.Name;
                    if (!info.Icon.empty()) {
                        nameLabel = info.Icon + " " + nameLabel;
                    }
                    ...
                    if (!info.Author.empty()) {
                        if (!info.Version.empty()) {
                            ImGui::TextDisabled("  %s  v%s", info.Author.c_str(), info.Version.c_str());
                        } else {
                            ImGui::TextDisabled("  %s", info.Author.c_str());
                        }
                    }
                    if (!info.License.empty()) {
                        ImGui::TextDisabled("  License: %s", info.License.c_str());
                    }
                    ...
                    if (entry->IsSigned() && entry->IsChecksumValid()) {
                        ...
                        const char* originLabel = " [Game]";
                        switch (origin) {
                            case Ship::KeyOrigin::User:
                                originColor = UIWidgets::Colors::Yellow;
                                originLabel = " [User]";
                                break;
                            case Ship::KeyOrigin::Game:
                                originColor = UIWidgets::Colors::Green;
                                originLabel = " [Game]";
                                break;
                            case Ship::KeyOrigin::System:
                                originColor = UIWidgets::Colors::White;
                                originLabel = " [System]";
                                break;
                        }
                        ImGui::TextColored(originColor, "  " ICON_FA_CHECK_CIRCLE " Signed%s", originLabel);
                    } else if (entry->IsChecksumValid()) {
                        ImGui::TextColored(UIWidgets::Colors::Yellow,
                                           "  " ICON_FA_EXCLAMATION_TRIANGLE " Unsigned (Caution)");
                    } else {
                    ...
                    if (!info.Website.empty()) {
                        if (UIWidgets::MenuItem(ICON_FA_EXTERNAL_LINK " Open Webpage")) {
                            SDL_OpenURL(info.Website.c_str());
                        }
                    }
                }
            }
...
void DrawDebugMenu() {
    if (UIWidgets::BeginMenu("Developer")) {
        ...
        UIWidgets::CVarCheckbox(
            ...
            { .tooltip = "Disables the game's Built-in Gamma Boost. Useful for modders", .defaultValue = false });
        ...
        UIWidgets::CVarCheckbox("L To Warp Zone", "gDebugWarpZone", { .tooltip = "Press L to get into the Warp Zone" });
        ...
        UIWidgets::CVarCheckbox("Disable Collision", "gDebugNoCollision", { .tooltip = "Disable vehicle collision" });

        UIWidgets::CVarCheckbox("Speed Control", "gDebugSpeedControl",
                                { .tooltip = "Arwing speed control. Use D-PAD Left and Right to Increase/Decrease the "
                                             "Arwing Speed, D-PAD Down to stop movement." });

        UIWidgets::CVarCheckbox("Debug Ending", "gDebugEnding", { .tooltip = "Jump to credits at the main menu" });

        UIWidgets::CVarCheckbox("Debug Pause", "gLToDebugPause", { .tooltip = "Press L to toggle Debug Pause" });
        ...
        if (CVarGetInteger(StringHelper::Sprintf("gCheckpoint.%d.Set", gCurrentLevel).c_str(), 0)) {
            ...
            if (UIWidgets::Button("Set Checkpoint")) {
                ...
                CVarSetInteger(StringHelper::Sprintf("gCheckpoint.%d.gSavedPathProgress", gCurrentLevel).c_str(),
                               gGroundSurface);
                CVarSetFloat(StringHelper::Sprintf("gCheckpoint.%d.gSavedPathProgress", gCurrentLevel).c_str(),
                             (-gPlayer->pos.z) - 250.0f);
                CVarSetInteger(StringHelper::Sprintf("gCheckpoint.%d.gSavedObjectLoadIndex", gCurrentLevel).c_str(),
                               gObjectLoadIndex);
                Ship::Context::GetInstance()->GetWindow()->GetGui()->SaveConsoleVariablesNextFrame();
            }
        }
