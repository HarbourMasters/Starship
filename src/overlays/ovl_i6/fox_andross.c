...
void Andross_801878A8() {
    ...
    gLevelObjects = LOAD_ASSET(&D_ANDROSS_C0356CC);
    ...
    for (scenery360 = gScenery360, i = 0; i < 1000; i++) {
        ...
        if (gLevelObjects[i].id < OBJ_SCENERY_MAX) {
            ...
            if (scenery360->obj.id == OBJ_SCENERY_131) {
                scenery360->info.hitbox = LOAD_ASSET(&D_ANDROSS_C038CCC);
            }
            scenery360++;
        }
    }
...
void Andross_80187C5C(void) {
    ...
    switch (RAND_INT(2.9f)) {
        case 0:
            gLevelObjects = LOAD_ASSET(&D_ANDROSS_C036310);
            break;
        case 1:
            gLevelObjects = LOAD_ASSET(&D_ANDROSS_C036B6C);
            break;
        default:
        case 2:
            gLevelObjects = LOAD_ASSET(&D_ANDROSS_C03733C);
            break;
    }
...
void Andross_80189098(Boss* boss) {
    ...
    Object_SetInfo(&actor->info, actor->obj.id);
    actor->info.hitbox = LOAD_ASSET(&gCubeHitbox150);
...
void Andross_80189B70(Boss* boss) {
    ...
    switch (boss->state) {
        ...
        case 2:
            ...
            break;
        case 11:
            boss->info.hitbox = LOAD_ASSET(&gNoHitbox);
    ...
    switch (scenery->state) {
        case 0:
            if (fabsf(scenery->obj.pos.z - gPlayer[0].trueZpos) < 1800.0f) {
                scenery->state = 1;
                scenery->info.hitbox = LOAD_ASSET(D_ANDROSS_C038AC4);
            }
            break;
        case 1:
